...
const Product = () => {
    ...
    .then(res => res.json())
    .then(data =>{
      const filterProducts = data.filter(filterProduct => filterProduct.advertise)
      setProducts(fi)})
    .catch((error) => {
      console.error(error)
    })
  ...
  <div className="hero-content flex-col lg:flex-row">
    <div className='text-center'>
      <h1 className='text-2xl font-extrabold'>Category</h1>
          ...
          return <ul>
            <li className='mt-5'><Link to='/product-category' className="link link-hover bg-emerald-300 p-2">{product.category}</Link></li>
          </ul>
          })}
      </div>
    <div>
